		...
		void polynomicForce(const vector<double>& position_x, vector<double>& acceleration_x){
			...
			for (int i=0; i<number_particles; i++){
				...
				if (i==number_particles-1){
					if (a != 0.0){Force += a*(position_x[i] - position_x[0]);}
					if (b != 0.0){Force += b*(position_x[i] - position_x[0])*(position_x[i] - position_x[0]);}
					if (c != 0.0){Force += c*(position_x[i] - position_x[0])*(position_x[i] - position_x[0])*(position_x[i] - position_x[0]);}
					acceleration_x[i] -= Force*mass_inverse;
					acceleration_x[0] += Force*mass_inverse;
				}
				else{
